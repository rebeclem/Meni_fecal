---
title: "Allison Meni Fecal Samples
"
...
setwd("~/Documents/Research/Meni/Meni2")
...
# If you get the error: "cannot open file './read_count.csv': No such file or directory Error in file(file, "rt") : cannot open the connection", then just copy and paste the first command into the console instead of running the whole block.
seq.results <- read.csv("./read_count_meni.csv")
...
PS_map.results <- read.csv("reads_bac_human.csv")
...
PS_map.results.long$Group_f = factor(PS_map.results.long$Group, levels=c('3','4','7','8','9','11','12','13','zymo'))
...
sampledata <- read.csv("meni_metadata.csv")
sampledata <- as.data.frame(sampledata)
colnames(sampledata) <- c("Samp","Group")
rownames(sampledata) <- sampledata[,1]
sampledata$Group_f = factor(sampledata$Group, levels=c('3','4','7','8','9','11','12','13','zymo'))
...
file.names = list.files(path = "./bac", pattern="*.tsv", full.names = TRUE)
...
# New way from BNN to use taxonimizer to get taxonomic info from NCBI easier.
...
patho.byclass.tr.f <- filter_taxa(patho.byclass.tr, function (x) max(x) > 1e-2, TRUE) #filter out phyla with < 1% relative abundance.
...
patho.by_class_plot <- patho.by_class_plot + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust=0.5, hjust=0))
patho.by_class_plot
ggsave("PS.by_class_max1.pdf", patho.by_class_plot)
```
...
patho.byspecies.tr.f <- filter_taxa(patho.byspecies.tr, function (x) max(x) > 1e-2, TRUE)
...
#ggsave("MPS.by_species_mean.pdf", patho.by_species_plot, width = 7, height = 7, units = "in")
```
